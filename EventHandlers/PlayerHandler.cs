    ...
    internal sealed class PlayerHandler
    {
        ...
        public void OnSpawned(SpawnedEventArgs ev)
        {
            ...
            if (ev.Player.Role.Team == Team.ClassD)
            {
                ...
                if (Class1.Instance.Config.ClassDSpawnItem == null)
                {
                    Log.Warn("No ClassDSpawnItem was set in the server config.");
                }
                else
                {
                    ev.Player.AddItem(Class1.Instance.Config.ClassDSpawnItem);
                }
            }
